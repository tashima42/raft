...
func (r *Raft) AppendEntries(req AppendEntriesRequest) (bool, int, error) {
	...
	for {
		...
		case <-r.ctx.Done():
			...
			return false, -1, errors.New("context canceled")
		default:
...
func (r *Raft) RequestVote(req RequestVoteRequest) (int, bool, error) {
	for {
		...
		case <-r.ctx.Done():
			r.logger.InfoContext(r.ctx, "cancel command received, closing.")
			return -1, false, errors.New("context canceled")
		default:
			...
			r.logger.InfoContext(r.ctx, fmt.Sprintf("request voted - voted for: %d", votedFor))
